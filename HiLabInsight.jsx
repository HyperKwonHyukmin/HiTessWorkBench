...
export default function HiLabInsight({ setCurrentMenu }) {
  ...
  const fetchIngestedDocs = async () => {
    try {
      ...
      if (res.data.documents) {
        setIngestedDocs(res.data.documents);
      }
    } catch (error) {
      console.error("문서 현황 로드 실패:", error);
    }
  };
  ...
  const handleSend = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/ai/chat`, { question: userMessage.text });
  ...
  const handleAuthSubmit = async (e) => {
    ...
    if (adminPassword === 'str_2006') {
      ...
      try {
        ...
        // 학습 시작 후 문서 현황을 주기적으로 갱신해주면 좋습니다 (여기서는 1회만 재호출)
        setTimeout(fetchIngestedDocs, 5000); 
      } catch (error) {
        alert("Ingest 요청에 실패했습니다.");
      } finally {
        setIsIngesting(false);
      }
    } else {
      setAuthError('권한이 없습니다. (비밀번호 불일치)');
    }
  };
      ...
      <div className="bg-white p-4 rounded-t-2xl border border-slate-200 border-b-0 shadow-sm flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Report Knowledge Assistant</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {/* 학습 현황 보기 버튼 */}
          <button 
            onClick={() => { fetchIngestedDocs(); setIsDocsModalOpen(true); }}
            className="flex items-center gap-2 px-4 py-2 text-sm font-bold bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 rounded-lg transition-colors shadow-sm"
          >
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-lg transition-colors shadow-sm ${isIngesting ? 'bg-slate-100 text-slate-400' : 'bg-indigo-50 text-indigo-600 border border-indigo-200 hover:bg-indigo-100'}`}
          >
            <Database size={16} />
            {isIngesting ? '백그라운드 학습 중...' : '지식 DB 업데이트'}
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-slate-50 p-6 overflow-y-auto border border-slate-200 custom-scrollbar flex flex-col gap-4">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 max-w-[80%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
            ...
            <div className={`p-4 rounded-2xl shadow-sm text-sm whitespace-pre-wrap leading-relaxed ${msg.role === 'user' ? 'bg-[#002554] text-white rounded-tr-sm' : 'bg-white text-slate-700 border border-slate-200 rounded-tl-sm'}`}>
              {msg.text}
            </div>
          </div>
        ))}
        {isLoading && (
          <div className="flex gap-3 max-w-[80%]">
            ...
            <div className="p-4 bg-white border border-slate-200 rounded-2xl rounded-tl-sm shadow-sm flex gap-2 items-center">
              <RefreshCw className="animate-spin text-slate-400" size={16} /> <span className="text-slate-500 text-sm">답변을 탐색하고 생성 중입니다...</span>
            </div>
          </div>
        )}
      ...
      <div className="bg-white p-4 rounded-b-2xl border border-slate-200 border-t-0 shadow-sm shrink-0">
        <form onSubmit={handleSend} className="relative flex items-center">
          <input 
            ...
            placeholder="학습된 문서를 바탕으로 궁금한 점을 질문해 보세요..." 
            className="w-full bg-slate-100 border border-slate-200 rounded-xl pl-4 pr-12 py-4 outline-none focus:border-purple-400 focus:bg-white transition-colors text-sm text-slate-800"
            disabled={isLoading}
          />
          <button 
            type="submit" 
            disabled={isLoading || !input.trim()}
            className="absolute right-2 p-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-slate-300 transition-colors cursor-pointer"
          >
            <Send size={18} />
          </button>
        </form>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Dialog.Panel className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] transform animate-fade-in-up">
              <div className="bg-[#002554] p-5 flex justify-between items-center text-white shrink-0">
                <Dialog.Title className="font-bold text-lg flex items-center gap-2">
                  <Database size={20} className="text-[#00E600]"/> 지식 DB 벡터 반영 현황
                </Dialog.Title>
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-slate-50">
                {Object.keys(ingestedDocs).length === 0 ? (
                  <div className="text-center py-10 text-slate-400">
                    ...
                    <p>학습된 문서가 없거나 정보를 불러올 수 없습니다.</p>
                  </div>
                ) : (
                      ...
                      <div key={idx} className="bg-white p-4 border border-slate-200 rounded-xl shadow-sm flex flex-col gap-3 hover:border-blue-400 transition-colors">
                        <div className="flex justify-between items-start">
                          <h3 className="font-bold text-slate-800 flex items-center gap-2">
                            <FileText size={16} className="text-blue-500"/> {filename}
                          </h3>
                        ...
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-xs text-slate-500 leading-relaxed font-mono line-clamp-2">
                          {info.preview || "미리보기 정보가 없습니다."}
                        </div>
                        {/* 간단한 프로그레스 바 UI (모두 완료된 상태를 가정) */}
                        <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden mt-1">
                          <div className="bg-gradient-to-r from-blue-400 to-emerald-400 h-full w-full"></div>
                        </div>
