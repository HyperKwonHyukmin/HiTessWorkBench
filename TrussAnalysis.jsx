...
export default function TrussAnalysis({ setCurrentMenu }) {
  ...
  const handleDrop = (e, type) => { e.preventDefault(); handleFile(e.dataTransfer.files[0], type); };
  const addLog = (message, type = 'info') => { const time = new Date().toLocaleTimeString(); setLogs(prev => [...prev, { time, message, type }]); };
      ...
      <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
        ...
        <div className="w-full lg:w-[400px] flex flex-col gap-5 shrink-0 overflow-y-auto pr-1 custom-scrollbar">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2"><Database size={16} className="text-blue-500"/> 1. Data Input</h3>
...
function TabButton({ active, onClick, icon: Icon, label, count }) {
  return <button onClick={onClick} className={`px-4 py-2.5 rounded-t-lg font-bold text-sm flex items-center gap-2 cursor-pointer ${active ? 'bg-white text-[#002554] border-t-2 border-t-[#002554]' : 'text-slate-500 hover:bg-slate-100'}`}><Icon size={16} /> {label}</button>;
}
